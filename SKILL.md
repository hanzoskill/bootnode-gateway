...
metadata:
  ...
  bot:
    emoji: "\U0001F916"
    requires:
      env: [BOOTNODE_WALLET_PRIVATE_KEY]
---
...
Use this skill when:
- The agent needs keyless blockchain access without a Bootnode account
...
If `BOOTNODE_API_KEY` is set, use the `bootnode-api` skill instead.

## Hard requirements

1. **Wallet required.** `BOOTNODE_WALLET_PRIVATE_KEY` must be set to an Ethereum private key that holds USDC on Base.
...
4. **If `BOOTNODE_API_KEY` is set**, prefer the `bootnode-api` skill (it's cheaper for high volume).

## Preflight checks

Before making any request, silently verify:
1. `BOOTNODE_WALLET_PRIVATE_KEY` is set and non-empty
2. If `BOOTNODE_API_KEY` is also set, warn that `bootnode-api` skill may be more cost-effective
...
| Item | Value |
|------|-------|
| Gateway URL | `https://gateway.bootno.de/v1` |
...
| Docs | https://bootno.de/docs/agents |
...
export BOOTNODE_WALLET_PRIVATE_KEY=0x...
```
...
curl -X POST https://gateway.bootno.de/v1/eth/mainnet \
...
### Using the Bootnode Agent SDK

```typescript
import { BootnodeAgent } from "@bootnode/agent-sdk"

const agent = new BootnodeAgent({
  privateKey: process.env.BOOTNODE_WALLET_PRIVATE_KEY,
})
...
Same endpoints as `bootnode-api` skill, but using gateway URL:

| Task | Endpoint |
|------|----------|
| JSON-RPC | `POST https://gateway.bootno.de/v1/{network}` |
| Token data | `GET https://gateway.bootno.de/v1/data/tokens/*` |
| NFT data | `GET https://gateway.bootno.de/v1/data/nfts/*` |
| Portfolio | `GET https://gateway.bootno.de/v1/data/portfolio` |
...
- Agent docs: https://bootno.de/docs/agents
